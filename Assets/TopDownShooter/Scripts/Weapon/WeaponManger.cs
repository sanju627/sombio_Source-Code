...
public class WeaponManger : MonoBehaviour
{
    ...
    {	
        ...
        SwitchMelee(PlayerPrefs.GetInt("Melee"));
    ...
    void Update()
    {
        if(!player.inCar && !player.inHelicopter)
        {
            ...
            withinStorage = Physics.CheckSphere(transform.position, radius, storageLayer);
    ...
    public void SwitchMelee(int choice)
    {
        for (int i = 0; i < Melee.Length; i++)
        {
            Melee[i].SetActive(false);
